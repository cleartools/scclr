...
def ref_pf_mean(counts, k):
    """Independent dense PF-to-mean (depth-target) reference: log1p(k*x/s), centered."""
...
def test_normalize_matches_numpy_reference():
    ...
    np.testing.assert_allclose(sclr.to_dense(), ref_pf_mean(counts, k), atol=1e-9)
...
def test_normalize_preserves_sparsity_pattern():
    ...
    # PFlog keeps the same nonzeros as the input (log1p(0)=0).
...
def test_scverse_dropin_anndata():
    ...
    scclr.pp.pflog(adata, target="mean")
    assert "pflog" in adata.layers
    assert "pflog_center" in adata.obs
    assert adata.uns["pflog"]["k"] is not None

    # The stored layer + center reconstruct PFlog exactly (normalization correctness
    # through the AnnData path).
    Y = adata.layers["pflog"].toarray() - adata.obs["pflog_center"].to_numpy()[:, None]
    np.testing.assert_allclose(Y, ref_pf_mean(counts, adata.uns["pflog"]["k"]), atol=1e-9)
...
def test_scverse_dropin_mudata():
    ...
    scclr.pp.pflog(mdata, target="mean")
    scclr.tl.pca(mdata, n_comps=3)
    for mod in mdata.mod.values():
        assert "pflog" in mod.layers
